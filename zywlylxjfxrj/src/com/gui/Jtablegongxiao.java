...
public class Jtablegongxiao {   //创建一个Dialog显示一下
	static class ModelDemo1 extends AbstractTableModel
	{
	       /**
		 * 
		 */
		private static final long serialVersionUID = 4402672798368688965L;
		private Vector<String[]> TableData;//用来存放表格数据的线性表
	       ...
	       public ModelDemo1(String[] cc)
	       {
	              ...
	              TableTitle.add("当归小分子");
	              TableTitle.add("当归靶标");
	      //        TableTitle.add("第三列");
	              //初始化3行数据，方便起见直接用String数组保存每一行的数据
	              String[] first = {"序号","共同小分子名称"};
	              TableData.add(first);
	              int l = 0;
	              for(int i = 0;i < cc.length;i++){
	            	  if(cc[i] == null){
		            	  ...
		            	  String []line = {l1,cc[i]};//i1表示序号，cc[i]为输出到表格的数据
		            	  TableData.add(line);
		            	  l++;
	            	  }
	              }
	...
	public static void huabiao(String[] str)
    { 
		  /*String[] strr = new String[10000];
		  System.arraycopy(str, 0, strr, 0, str.length);*/
		  ModelDemo1 myModel=  new ModelDemo1(str); 
          JTable table = new JTable(myModel);
          JFrame jf = new JFrame();
          jf.setTitle("两种药物共同小分子");
